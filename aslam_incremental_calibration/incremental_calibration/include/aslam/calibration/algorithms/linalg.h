...
namespace aslam
{
  namespace calibration
  {
     ...
     */
    cholmod_sparse *columnSubmatrix(cholmod_sparse *A, std::ptrdiff_t colStartIdx, std::ptrdiff_t colEndIdx, cholmod_common *cholmod);
    /**
     ...
     * \param[in] cholmod cholmod workspace
     */
    cholmod_dense *columnScalingMatrix(cholmod_sparse *A, cholmod_common *cholmod, double eps = std::numeric_limits<double>::epsilon());
    /**
    ...
    cholmod_sparse *eigenDenseToCholmodSparseCopy(const Eigen::MatrixXd &in,
                                                  cholmod_common *cholmod, double eps = std::numeric_limits<double>::epsilon());
    /**
    ...
    cholmod_dense *reduceRightHandSide(SuiteSparseQR_factorization<double> *
                                           factor,
                                       cholmod_sparse *A_rt, cholmod_sparse *A_rtQ, cholmod_dense *b,
                                       cholmod_common *cholmod);
    /**
     ...
     * \param[out] x result
     */
    void solveSVD(const cholmod_dense *b, const Eigen::VectorXd &sv, const Eigen::MatrixXd &U, const Eigen::MatrixXd &V, std::ptrdiff_t rank,
                  Eigen::VectorXd &x);
    /**
