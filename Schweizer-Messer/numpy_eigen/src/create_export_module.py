    ...
    for d1x in range(0, len(dims)):
        for d2x in range(0, len(dims)):
            for Tx in range(0, len(types)):
                cmakef.write(
                    "\n\tsrc/autogen_module/import_%s_%s_%s.cpp"
                    % (dimTags[d1x], dimTags[d2x], typeTags[Tx])
                )
                with open(
                    os.path.join(
                        "autogen_module/import_%s_%s_%s.cpp"
                        % (dimTags[d1x], dimTags[d2x], typeTags[Tx])
                    ),
                    "w",
                ) as f:
                    ...
                    f.write("\n")
                    f.write(
                        "void import_%s_%s_%s()\n"
                        % (dimTags[d1x], dimTags[d2x], typeTags[Tx])
                    )
                    f.write("{\n")
                    f.write(
                        "\tNumpyEigenConverter<Eigen::Matrix< %s, %s, %s > >::register_converter();\n"
...
with open("autogen_test_files.cmake", "w") as cmakef:
    ...
    for d1x in range(0, len(dims)):
        for d2x in range(0, len(dims)):
            for Tx in range(0, len(types)):
                cmakef.write(
                    "\n\tsrc/autogen_test_module/test_%s_%s_%s.cpp"
                    % (dimTags[d1x], dimTags[d2x], typeTags[Tx])
                )
                with open(
                    os.path.join(
                        "autogen_test_module/test_%s_%s_%s.cpp"
                        % (dimTags[d1x], dimTags[d2x], typeTags[Tx])
                    ),
                    "w",
                ) as f:
                    td1 = d1x
                    td2 = d2x
                    f.write("#include <Eigen3/Core>\n\n")
                    f.write("#include <numpy_eigen/boost_python_headers.hpp>\n")
                    f.write(
                        "Eigen::Matrix<%s, %s, %s> test_%s_%s_%s(const Eigen::Matrix<%s, %s, %s> & M)\n"
                        % (
                            types[Tx],
                            dims[d1x],
                            dims[d2x],
                    ...
                    f.write("\n")
                    f.write(
                        "void export_%s_%s_%s()\n"
                        % (typeTags[Tx], dimTags[td1], dimTags[td2])
                    )
                    f.write("{\n")
                    f.write(
                        '\tboost::python::def("test_%s_%s_%s",test_%s_%s_%s);\n'
                        % (
                            typeTags[Tx],
                            dimTags[td1],
                            dimTags[td2],
